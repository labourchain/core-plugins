...
`dependencies[]` 与 runtime inject 的语义边界由 `core-runtime-abi.md` 定义；实际一致性检查由两个外部边界负责：
...
当前 Core artifact build/release 不实现通用 dependency/inject consistency validator。
...
runtime dependency consistency validation
