...
大型模型、图片、视频、地图、词典、数据集或资源包应优先拆到更高层 Asset/Runtime。构建工具建议在 executable artifact 大约超过 500 KiB 时给 bundle-size warning；这不是共识有效性限制。

## 当前审查状态

`core.plugin` 已实现 Plugin identity、external/embedded artifact verification。

`core.record`、`core.block` 与 Genesis 的部分 identity/signature/ordering 规则仍按旧 Service 做 source-first review；此前引入的 Plugin activation/S0/Repository issuer 状态机已经撤销，不应作为实现前提。
